...
def get_hierarchical_mask(
    ...
    for label in oois:
        ...
        hierarchical_target[:,:,i] = np.logical_or(hierarchical_target[:,:,i], label_mask)
...
def show_annotations(json_file):
    ...
    for label in oois:
        ...
        for points in coois[label]:
            ...
            if len(points) >= 3:
                ...
                patch = pylab.Rectangle((x, y), width, height, color=color, lw=2, fill=False)
                ax.add_patch(patch)
                ...
                patch = matplotlib.patches.Ellipse((c, r), major, minor, -np.degrees(orientation - np.pi/2), color=color, fill=False, lw=2)
                ax.add_patch(patch)
    ...
    if np.any(aoi):
        ...
        patch = pylab.Circle(epooa[::-1], radius=5, color=sns.xkcd_rgb[colors_for_labels['extreme']])
        ax.add_patch(patch)
        patch = pylab.Circle(epioa[::-1], radius=5, color=sns.xkcd_rgb[colors_for_labels['start_likely']])
        ax.add_patch(patch)
    ...
    if np.any(support):
        ...
        patch = pylab.Circle(epioa[::-1], radius=5, color=sns.xkcd_rgb[colors_for_labels['start_possible']])
        ax.add_patch(patch)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-j', '--json', default='double_clicks_100161_Wed_Jul_10_210910_2019_double_click_zoom_2_y_529_x_606.json', type=str, help='path to the json file containing sample annotation')
